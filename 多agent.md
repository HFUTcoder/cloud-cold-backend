这篇文章我就想带你从｢源码｣视角，把 Claude Code 的多 Agent 机制彻底讲明白，目标是让你看完能同时 get 三个问题：
...
代码分析Agent ·代码理解 ·依赖分析 代码改造Agent ·重构代化 ·代码修改
·结构理 ·Bug修复
搜索检索Agent CLAUDE <> 测试验证Agent
·语义搜索 8 单元测试
·文档检索 ·示例叠找 ·结果验证 集成测试

工具集(Tools) 通信与协作 任务完成! f ruself)
终城 文件读写 网络语求数据库 * 生成代码变更测试服
```
...
目标:了解解决方案
如何实现XXX功能 
搜索结果 官方文档 U 3 else: raise ValueError
超
"invalid request"
技术博客 M 6 评审
GitHub issue 目标:代码质量把关
 
8
 最佳实践 </> return True 边 0
BUG 什么时 架构模式
头大...
```
...
### 1.2 Multi-Agent 的三种常见形态
绕开花哨的术语，Multi-Agent 系统在工业界落地时，一般就三种形态。

```
父子型 平级协作型 主从型
Agent Agent Worker
Agent Agent Worker Coordinator Worker
Agent Agent Agent Agent Agent Worker Worker
```
...
| Claude Code的机制 | 常见Multi-Agent形态 | 特点 |
|------------------|--------------------|------|
| 常规Subagent | 树状结构(父子型) | 轻量、灵活，继承上下文，适合层级分解任务。 |
| Fork Subagent | 优化的Fork结构 | 独立进程，拥有独立上下文，适合并行执行与隔离任务。 |
| Coordinator | 星型结构(主从型) | 全局视角，任务分配与进度跟踪，负责协调与整合结果。 |

### 1.3 subagent 在 Claude Code 里到底长啥样?
...
<>
·指定能力与边界 ·传递上下文与目标 ·创建subagent Q搜索文件 读取文件
·获取结果
独立上下文 独立上下文
 ·工具结果与中间状态 通过Agent工具， 主agent派发并运行 独立的subagent实例 ·专注手探索任务 只读信息与搜索结果 ·与主agent隔离
```
...
```
工具隔离 上下文隔离
为每个子Agent分配独立的工具集 父子Agent拥有独立的上下文
允许使用 禁止使用 父context 子context
系统提示 系统提示
Agent A 文件读写 搜索 终端 网络请求 数拥库 选择性复制
自→
允许使用 禁止使用 对话历史 … 只复制必要的 数据和指令 对话历史
Agent B 文件读写搜索 网络请求 终端 数据序 项目文件 REAOME.nd 阻止状态变更 项目文件(只读) README.nd
允许使用 禁止使用 src/main.py src/main.py
Agent C 文件读写 终端 数据库 搜索 网络请求 {}交量、缓存、会 运行状态 运行状态(独立) {}文量、缓存、会话等

允许使用的工具 禁止使用的工具
```

### 2.1 第一维度：给子 agent 发一个定制工具箱
...
第三道门反过来，是｢后台异步 agent 走白名单｣。这类 agent 是完全后台跑的，没法跟用户交互，所以只准用事先圈定好的一小批工具(读文件、搜代码、执行命令、编辑文件这些)。
...
AskUser X
星 X 自定义工具箱
默认禁止的工具 自定义加严规则 允许的异步工具集合 Subagent

Plan 仅先特定前 车限 手数校验
防止危险操作与无限退归 按需收紧，更可控 只放行安全，可异步的工具
三道门层层过滤，确保安全可控的Muli-Agent工具调用
```
...
| 工具 | 主agent | 内置sub | 定义sub | 异步sub |
|------|---------|---------|---------|---------|
| Bash | 几乎所有工具权限 | 主agent拥有 |  |  |
| Read |  | sub agent经过过滤 | 仅保留必要权限 |  |
| TodoWrite |  |  |  |  |
| AskUser | 允许 | 按过滤规则决定(部分允许) | 不允许 |  |
|  |  |  | 异步sub agent |  |
|  |  |  | 仅允许白名单工具 |  |
...
任务通路
全局状态 (禁止修改) DEPTH +1
复制一份钥匙， 生活类比: 各自使用不影响。 公共日记本 锁起来，不能乱写。 生活类比: 生活类比: 保留投递口， 可以投递新任务。 自己有独立身份， 生活类比: 层级深度+1.
克隆readFileState 关闭setAppState 保留任务通路 独立ID+深度+1
```
...
```
父子context隔离
父context readFilleState 克隆 子context readFileState
已读取文件缓存 已读取文件缓存
关闭
全局状态(可变更) setAppState setAppState 全局状态(只读)
setAppStateForTasks 任务注册通道 保留 任务注册通道(可用) setAppStateForTasks
独立
 H
身份与链路信息 身份与链路信息(独立)
```
...
```
函数调用 消息驱动
同步阻塞:调用后必须等待结果 异步解:发送消息后可继续执行
父agent subagent 父agent subagent inbox
还没 完吗... doTask(args) 调用函数 (task: xxxx) 发送消息 有新消息!

父agent傻等 返回结果 result 开始处理任务... 继续处理自己的任务 读取消息
阻塞等待中... 90% 发送结果消息 { result:xxxx) 开始处理任务...
在结果返回之前，父agent只能干等， 无法做其他事情。 父agent无需等待，可并行处理其他事情， 通过消息完成异步协作，系统更灵活!
```
...
最直觉的做法是：给主线程发一个｢工具返回结果｣事件。但 Claude Code 玩得更骚气，它的设计是：把完成通知拼成一段 XML，伪装成一条用户消息，塞给父 agent 的对话历史。父 agent 那边看到的就像用户发了一条新消息过来，长这样：
```xml
<task-notification>
...
### 4.3 一个有意思的细节：system prompt 不重新生成
...
### 4.4 什么时候用 Fork，什么时候用常规 subagent?
...
### 5.6 Continue vs Spawn：老 worker 还是新 worker?
